...
public class IRubricManagerHibernateImpl extends HibernateDaoSupport
    ...
    public GradableObjectRubric getGradableObjectRubric(Long gradableObjectId) {
        List<GradableObjectRubric> results = getHibernateTemplate().getSessionFactory().getCurrentSession()
                .createQuery("from " + GradableObjectRubric.class.getName() + " as gr where gr.gradableObjectId =: goid")
                .setParameter("goid", gradableObjectId)
                .getResultList();
        if (results.size() > 0) {
            return (GradableObjectRubric) results.get(0);
        }
        return null;
    }
